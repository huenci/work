...
public class DateUtil {
	...
	* @author fh
    ...
    /**
     * 时间相减得到分钟
     * @param beginDate 前一个时间
     * @param endDate 后一个时间
     * @return
     */
    public static long getDaySubMin(Date beginDate,Date endDate){
        long min=0;
        min=(endDate.getTime()-beginDate.getTime())/(1000*60);
        return min;
    }
     ...
     * @return
     */
    public static Date getAfterDayDate(String startDate,String days) {
    	Date start = fomatDate(startDate);
    	int daysInt = Integer.parseInt(days);
    	
        Calendar canlendar = Calendar.getInstance(); // java.util包
        canlendar.setTime(start);
        canlendar.add(Calendar.DATE, daysInt-1); // 日期减 如果不够减会将月变动
        Date date = canlendar.getTime();
        /*SimpleDateFormat sdfd = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        String dateStr = sdfd.format(date);*/
    ...
    public static boolean getstartendtime(String startTime ,String endTime){
    	boolean s = false;
...
//		int second = now.getSeconds();
		if(startList.length>2 && endList.length > 2){
			Integer startH = Integer.valueOf(startList[0]);
//			Integer startM = Integer.valueOf(startList[1]);
//			Integer startS = Integer.valueOf(startList[2]);
			Integer endH = Integer.valueOf(endList[0]);
//			Integer endM = Integer.valueOf(endList[1]);
//			Integer endS = Integer.valueOf(endList[2]);
			if(endH >= startH){
				if(startH <= hours && hours <= endH){
					s =true;
				}
			}else{
				if(!(endH < hours && hours < startH)){
					s =true;
				}
			}
		}
...
//		System.err.println("结束");、
    	Date date = getAfterDayDate("2018-12-23","2");
